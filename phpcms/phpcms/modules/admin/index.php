...
class index extends admin {
	...
	$todaycount = $this->account_db->count("register_time>'$today' and register_time<'$tomorrow'");
	//当前在线
	$sql = "select count(1) as count from t_dz_online_player";
	$onlineplayers = $this->account_db->get_one_by_sql($sql);
	
	//售出转世
	$sql = "select sum(product_price) as product_price from t_dz_consume_history where '$today'<order_time<'$tomorrow' ";
	$diamond = $this->account_db->get_array_by_sql($sql);
//昨日
$yesterdaycount = $this->account_db->count("register_time>'$yesterday' and register_time<'$today'");

//售出转世
	$sql = "select sum(product_price) as product_price from t_dz_consume_history where '$yesterday'<order_time<'$today' ";
	$diamondyes = $this->account_db->get_array_by_sql($sql);
}
		/*检测框架目录可写性*/
